...
- [x] **Step 1: Write failing tests** for status phase persistence/normalization, `room_status` waiting on a live agent and pending request, and MCP exposure of `status_set`/`room_status`.
- [x] **Step 2: Run the focused tests** and confirm they fail because the new phase/details/tool interfaces do not exist.
- [x] **Step 3: Implement the locked status record extension** in `bus.py`, preserving old string reads and expiry behavior.
- [x] **Step 4: Implement `room_status` and validated `status_set`** in `server.py`, deriving `waiting_for` from unanswered request messages and process health from existing wake metadata.
...
- [x] **Step 1: Write failing tests** for reply-to-completed, initial/wake active phases, and terminal failure phases.
- [x] **Step 2: Run those tests** and confirm the current implementation leaves status `online`/`busy` without a lifecycle phase.
- [x] **Step 3: Add the smallest server lifecycle helpers** and call them at spawn, reply, rate-limit, no-reply, dead-wake, and stuck-wake boundaries.
- [x] **Step 4: Run the focused lifecycle tests** and confirm they pass without changing wake deduplication.
...
- [x] **Step 1: Write failing prompt tests** asserting the lifecycle/wait rules appear in each prompt family.
- [x] **Step 2: Run the prompt tests** and confirm they fail on the current prompts.
- [x] **Step 3: Add one shared lifecycle protocol block** and include it in every CLI prompt; update runner prompts and user-facing docs.
- [x] **Step 4: Run prompt tests, the full Python suite, and JS syntax verification.**
